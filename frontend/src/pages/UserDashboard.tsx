...
interface UserOverview {
  ...
  moodAverage: number;
  streakDays: number;
  upcomingSessions: { title: string; time: string }[];
}
...
export default function UserDashboard() {
  ...
  const fetchOverview = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/user/overview?email=${encodeURIComponent(user.email)}`);
      if (!response.ok) {
        setOverview(null);
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {/* User overview cards - Show After Modules */}
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-10">
          <Card className="p-6">
            <p className="text-sm text-muted-foreground">Completed tools</p>
            <p className="mt-2 text-3xl font-semibold">{overviewLoading ? '...' : overview?.totalTodos ?? 0}</p>
            <p className="text-sm text-muted-foreground mt-1">Daily habits tracked</p>
          </Card>

          <Card className="p-6">
            <p className="text-sm text-muted-foreground">Chats</p>
            <p className="mt-2 text-3xl font-semibold">{overviewLoading ? '...' : overview?.totalMessages ?? 0}</p>
            <p className="text-sm text-muted-foreground mt-1">Messages with support tools</p>
          </Card>

          <Card className="p-6">
            <p className="text-sm text-muted-foreground">Mood score</p>
            <p className="mt-2 text-3xl font-semibold">{overviewLoading ? '...' : overview?.moodAverage?.toFixed(1) ?? '—'}</p>
            <p className="text-sm text-muted-foreground mt-1">Average this month</p>
          </Card>

          <Card className="p-6">
            <p className="text-sm text-muted-foreground">Streak</p>
            <p className="mt-2 text-3xl font-semibold">{overviewLoading ? '...' : overview?.streakDays ?? 0}d</p>
            <p className="text-sm text-muted-foreground mt-1">Current logging streak</p>
          </Card>
        </div>
        ...
        >
          <Card className="p-8 text-center bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Button onClick={() => setActiveFeature('breathing')}>
                <Wind className="w-4 h-4 mr-2" />
                Start Breathing
              </Button>
              <Button variant="outline" onClick={() => setActiveFeature('chat')}>
                <MessageCircle className="w-4 h-4 mr-2" />
                Chat with AI
              </Button>
              <Button variant="outline" onClick={() => setActiveFeature('journal')}>
